...
use crate::{
    components::{
        ...
        PlayerController, PropulsionState, SpatialComponent, TaskComponent, TrimCondition,
        TrimRequest,
    },
    ...
    fn spawn_aircraft(
        ...
        for config in configs {
            ...
            if config.trim_condition.is_none() {
                info!(
                    "No explicit trim condition provided for '{}'. Requesting default trim.",
                    config.name
                );
                ...
                if initial_speed > 1e-6 {
                    ...
                    info!(
                        "Sent TrimRequest (StraightAndLevel @ {:.2} m/s) for entity {:?}.",
                        initial_speed, entity
                    );
                } else {
                ...
                info!(
                    "Using explicit trim condition provided in config for '{}'.",
                    config.name
                );
            }
        }
    ...
    fn from_config(config: &FullAircraftConfig) -> Self {
        ...
        let (position, speed, heading) = match &config.start_config {
            ...
            crate::components::StartConfig::Fixed(fixed_config) => {
                info!(
                    "Using fixed start config: pos={:?}, speed={}, heading_rad={}",
            ...
            crate::components::StartConfig::Random(random_config) => {
                ...
                info!(
                    "Generated start config: pos={:?}, speed={}, heading_rad={}",
                    pos, spd, head_rad
                );
                (pos, spd, head_rad)
            }
        ...
        let velocity = Vector3::new(speed * heading.sin(), 0.0, speed * heading.cos());
        // Attitude calculation: Apply negative heading for clockwise rotation around Y-axis
        let attitude = UnitQuaternion::from_axis_angle(&Vector3::y_axis(), -heading);
