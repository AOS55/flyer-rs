...
/// Resource to store terrain rendering configuration
...
pub fn terrain_visual_update_system(
    ...
    render_config: Res<TerrainRenderConfig>,
) {
    for (_chunk_entity, chunk, children) in chunks.iter_mut() {
        update_chunk_visuals(chunk, children, &mut tiles, &terrain_assets, &render_config);
...
fn update_chunk_visuals(
    ...
    render_config: &TerrainRenderConfig,
) {
    ...
    for (child_index, &child_entity) in children.iter().enumerate() {
        if let Ok((mut sprite, mut transform, _tile)) = tiles.get_mut(child_entity) {
            ...
            // Update position
            transform.translation = Vec3::new(x, y, get_biome_layer(chunk.biome_map[child_index]));
...
fn get_biome_layer(biome: BiomeType) -> f32 {
    match biome {
        BiomeType::Water => 0.0,
        BiomeType::Sand => 1.0,
        BiomeType::Grass => 2.0,
        ...
        BiomeType::Crops => 2.0,
        BiomeType::Orchard => 3.0,
    }
}

// Plugin to organize terrain rendering systems
pub struct TerrainRenderPlugin;

impl Plugin for TerrainRenderPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<TerrainRenderConfig>()
            .add_systems(Update, terrain_visual_update_system);
    }
}
