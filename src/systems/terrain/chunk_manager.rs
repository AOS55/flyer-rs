...
use crate::systems::terrain::generator::*;
...
fn get_visible_chunks(
    ...
    if let Ok((camera_transform, projection)) = camera_query.get_single() {
        ...
        let chunks_to_load = window_chunks_x.max(window_chunks_y) + 1;

        let center_chunk = IVec2::new(
            (camera_pos.x / chunk_size_world).floor() as i32,
            (camera_pos.y / chunk_size_world).floor() as i32,
        );
...
/// Helper function to determine which chunks should be visible
// fn get_visible_chunks(
//     camera_query: &Query<(&Transform, &OrthographicProjection), With<Camera2d>>,
//     terrain_state: &TerrainState,
// ) -> HashSet<IVec2> {
...
//         let view_distance = projection.scale * 1.5; // Add some margin
//         let chunk_size_world = terrain_state.chunk_size as f32 * terrain_state.scale;

//         let (window_width, window_height) = (800.0, 600.0); // Get from window resource
//         let chunks_x = (window_width * projection.scale / chunk_size_world).ceil() as i32;
//         let chunks_y = (window_height * projection.scale / chunk_size_world).ceil() as i32;

//         // let chunks_to_load = (view_distance / chunk_size_world).ceil() as i32;
//         let chunks_to_load = chunks_x.max(chunks_y) + 2;
//         let center_chunk = IVec2::new(
//             (camera_pos.x / chunk_size_world).floor() as i32,
//             (camera_pos.y / chunk_size_world).floor() as i32,
//         );
...
//                     (center_chunk.x as i32 + x) as i32,
//                     (center_chunk.y as i32 + y) as i32,
//                 );

//                 // Convert to i32 for signed arithmetic first
//                 let chunk_x = (center_chunk.x as i32 + x) as i32;
//                 let chunk_y = (center_chunk.y as i32 + y) as i32;

//                 // Check world bounds
//                 // if chunk_pos.x < terrain_state.world_size.x
//                 //     && chunk_pos.y < terrain_state.world_size.y
//                 // {
//                 //     visible.insert(chunk_pos);
//                 // }
//                 let world_bounds = 1000i32; // Or use terrain_state.world_size.x/y
//                 if chunk_x >= -world_bounds
//                     && chunk_x < world_bounds
//                     && chunk_y >= -world_bounds
//                     && chunk_y < world_bounds
//                 {
//                     // Convert back to u32 using rem_euclid for proper wrapping
//                     let wrapped_x = chunk_x.rem_euclid(world_bounds) as i32;
//                     let wrapped_y = chunk_y.rem_euclid(world_bounds) as i32;
//                     visible.insert(IVec2::new(wrapped_x, wrapped_y));
//                 }

//                 // if chunk_pos.x.abs() < world_bounds && chunk_pos.y.abs() < world_bounds {
//                 //     visible.insert(UVec2::new(
//                 //         chunk_pos.x.rem_euclid(world_bounds as i32) as u32,
//                 //         chunk_pos.y.rem_euclid(world_bounds as i32) as u32,
//                 //     ));
//                 // }
//             }
//         }
...
fn spawn_chunk(
    ...
    let chunk_entity = commands
        .spawn(TerrainChunkComponent::new(
            position,
            terrain_state.chunk_size,
        ))
        .id();
    ...
    let world_pos = Vec2::new(
        position.x as f32 * terrain_state.chunk_size as f32 * tile_size,
        position.y as f32 * terrain_state.chunk_size as f32 * tile_size,
    );
        ...
        for x in 0..chunk_size {
            ...
            commands
                .spawn((
                    ...
                    Transform::from_translation(tile_pos.extend(0.0)).with_scale(Vec3::splat(1.0)),
                ))
                .set_parent(chunk_entity);

            // Logging for debugging
            info!("Spawning tile at position: {:?}", tile_pos);
        }
    }
}
