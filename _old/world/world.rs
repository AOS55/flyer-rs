...
use crate::terrain::{RandomFuncs, StaticObject, Terrain, TerrainConfig, Tile};
use crate::world::{Camera, Settings};
...
impl World {
    ...
    pub fn update_settings(
        ...
        self.settings = Settings::new(simulation_frequency, policy_frequency, render_frequency)
    }
    ...
    pub fn create_map(
        ...
        self.origin = Vec2::new(
            self.scale * (area[0] as f32 / 2.0),
            self.scale * (area[1] as f32 / 2.0),
        );
        ...
        let (tiles, objects) = match fs::File::open(&config_path) {
            Ok(mut file) => {
                ...
                let t_data: Result<TerrainData, serde_json::Error> =
        ...
        let tile_dir: Vec<_> = match fs::read_dir(&path) {
            ...
            Err(_td) => {
                eprintln!(
                    "Tiles dir not found in context, path is: {}",
        ...
        let so_dir: Vec<_> = match fs::read_dir(&path) {
            Ok(so) => so.filter_map(|entry| Some(entry.ok()?.path())).collect(),
            Err(_so) => {
                eprintln!(
                    "Object dir not found in context, path is: {}",
                    path.as_path().display()
                );
                std::process::exit(1);
            }
    ...
    #[allow(dead_code)]
    pub fn set_screen_dims(&mut self, width: f32, height: f32) {
    ...
    #[allow(dead_code)]
    pub fn set_assets_dir(&mut self, assets_dir: PathBuf) {
        self.assets_dir = assets_dir;
    }

    #[allow(dead_code)]
    pub fn set_terrain_data_dir(&mut self, terrain_data_dir: PathBuf) {
...
impl World {
    ...
    fn world_render(&mut self) -> Pixmap {
        ...
        let mut canvas =
            Pixmap::new(self.screen_dims[0] as u32, self.screen_dims[1] as u32).unwrap();
        let paint = PixmapPaint::default();

        // Calcuate the center of the screen and how much to transform each pixel by
        let center = Vec2::new(
            self.camera.x as f32 + self.origin[0],
            self.camera.y as f32 + self.origin[1],
        ); // center of image in [m]
        ...
        // Render tiles
        let render_results: Vec<(Pixmap, Transform)> = self
            .tiles
            .par_iter()
        ...
        let render_results = self
            .objects
            .par_iter()
            .filter_map(|object: &StaticObject| {
                    ...
                    && pix_pos[1] < self.screen_dims[1] + 50.0
                {
                    ...
                    let transform: Transform = Transform::from_row(
                        scale[0] / 16.0,
                        0.0,
                        0.0,
                        scale[1] / 16.0,
        ...
        match &self.runway {
            Some(runway) => {
                ...
                let scale = Vec2::new(
                    scaling_ratio[0] * (runway.dims[0] / 33.0),
                    scaling_ratio[1] * (runway.dims[1] / 1500.0),
                ); // [33.0, 1500.0] comes from the native image dimensions
    ...
    fn aircraft_render(&mut self) -> Pixmap {
        ...
        let horizontal_screen_center = Vec2::new(
            screen_center[0],
            (split_fraction / 2.0) * self.screen_dims[1],
        );
        let vertical_screen_center = Vec2::new(
            screen_center[0],
            (1.0 - ((1.0 - split_fraction) / 2.0)) * self.screen_dims[1],
        );
        ...
        let horizontal_pixel_x_pos =
            horizontal_screen_center.x - horizontal_object.width() as f32 / 2.0;
        let horizontal_pixel_y_pos =
            horizontal_screen_center.y - horizontal_object.height() as f32 / 2.0;
        let horizontal_transform = Transform::from_row(
            1.0,
            0.0,
            0.0,
            1.0,
            horizontal_pixel_x_pos,
            horizontal_pixel_y_pos,
        );
        let horizontal_transform = horizontal_transform.post_rotate_at(
            heading as f32 * 180.0 / std::f32::consts::PI,
            horizontal_screen_center.x,
            horizontal_screen_center.y,
        );

        // Add vertical aircraft
        let vertical_object = &self.object_map["t67v"];
        let pitch = -1.0 * self.vehicles[0].attitude().euler_angles().1;
        let vertical_transform = Transform::from_row(
            1.0,
            0.0,
            0.0,
            1.0,
            vertical_screen_center.x - vertical_object.width() as f32 / 2.0,
            vertical_screen_center.y - vertical_object.height() as f32 / 2.0,
        );
        let vertical_transform = vertical_transform.post_rotate_at(
            pitch as f32 * 180.0 / std::f32::consts::PI,
            vertical_screen_center.x,
            vertical_screen_center.y,
        );
        ...
        for pos in self.pos_log.iter().rev() {
            ...
            if horizontal_path_y_pos < split_point {
                let horizontal_point =
                    PathBuilder::from_circle(horizontal_path_x_pos, horizontal_path_y_pos, 3.0)
                        .unwrap();
                canvas.fill_path(
                    &horizontal_point,
                    &trace_paint,
    ...
    fn fixed_aircraft_render(&mut self) -> Pixmap {
        ...
        let scale = Vec2::new(
            self.screen_dims[0] / (self.area[0] as f32 * 16.0),
            self.screen_dims[1] / (self.area[1] as f32 * 16.0),
        );
        ...
        println!(
            "horizontal_pixel_x_pos: {}, horizontal_pixel_y_pos: {}",
            aircraft_pixel_x_pos, aircraft_pixel_y_pos
        );
        let horizontal_transform = Transform::from_row(
            1.0,
            0.0,
            0.0,
            1.0,
            aircraft_pixel_x_pos,
            aircraft_pixel_y_pos,
        );
        let horizontal_transform = horizontal_transform.post_rotate_at(
            (heading as f32 * 180.0 / std::f32::consts::PI) + 90.0,
            aircraft_pixel_x_pos,
            aircraft_pixel_y_pos,
        );
        canvas.draw_pixmap(
        ...
        match &self.goal {
            Some(goal) => {
                ...
                let goal_point =
                    PathBuilder::from_circle(goal_pix_x_pos, goal_pix_y_pos, 10.0).unwrap();
                canvas.fill_path(
                    &goal_point,
                    &goal_paint,
                    FillRule::Winding,
